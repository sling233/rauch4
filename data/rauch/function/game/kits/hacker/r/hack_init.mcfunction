...
execute as @s[scores={kit=8..9}] run function rauch:game/mechanics/set_armor_body
#execute as @s[scores={kit=1}] run function rauch:game/kits/bolt/q/remove_hit_id
execute at @s[scores={kit=2}] as @e[type=marker,tag=zarzahn] if score @s pnum = @p pnum run kill @s
execute at @s[scores={kit=2,zarzahn_hooking=0..}] run function rauch:game/kits/zarzahn/r/cancel_hook_zar
