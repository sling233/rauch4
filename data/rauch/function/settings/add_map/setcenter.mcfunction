...
data modify storage map_data maps[0].center set value [0d,0d,0d]
execute store result storage map_data maps[0].center[0] double 0.1 run scoreboard players get Global t_posx
execute store result storage map_data maps[0].center[1] double 0.1 run scoreboard players get Global t_posy
execute store result storage map_data maps[0].center[2] double 0.1 run scoreboard players get Global t_posz

#tellraw @s {"text":"----------------------------------------------------", "color":"yellow"}
tellraw @s [{"text":"Center location set to: ", "color":"yellow"},{"nbt":"maps[0].center[0]","storage":"map_data","color":"light_purple"},{"text":", "},{"nbt":"maps[0].center[1]","storage":"map_data","color":"light_purple"},{"text":", "},{"nbt":"maps[0].center[2]","storage":"map_data","color":"light_purple"}]
#tellraw @s {"text":"----------------------------------------------------", "color":"yellow"}
execute at @s run playsound minecraft:block.tripwire.click_off master @s ~ ~ ~ 1
function rauch:settings/add_map/givecenter
