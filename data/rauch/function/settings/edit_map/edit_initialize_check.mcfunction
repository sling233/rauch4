...
$execute as @s[tag=!temp1,tag=!temp2,tag=!temp3] run function rauch:settings/edit_map/edit_initialize {"map_id":$(map_id)}

execute as @s[tag=temp1] run tellraw @s {"text":"Only one Admin at a time can edit maps","color":"red"}
execute as @s[tag=temp3] run tellraw @s {"text":"Cannot edit maps while someone is editing the lobby","color":"red"}
