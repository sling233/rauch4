...
execute if score t_current_version temp = t_latest_version temp run tellraw @a {"text":"No Update found","color":"yellow"}
execute unless score t_current_version temp = t_latest_version temp run tellraw @a [{"text":"An Update is available for the prebuilt lobby. Click ","color":"yellow"},\
