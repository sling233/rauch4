# called by player function if the player swaps the weapon into their offhand, is neither hacked nor stunned and is not sneaking

execute as @s[scores={kit=1}] run return run function rauch:game/kits/bolt/f
execute as @s[scores={kit=2}] run return run function rauch:game/kits/zarzahn/f
...
execute as @s[scores={kit=8}] run return run function rauch:game/kits/flyer/f
execute as @s[scores={kit=9}] run return run function rauch:game/kits/wokkaman/f


tellraw @s [{text:"",color:"red"},"Error when trying to run f function for ",{"selector":"@s"},": not implemented for kit id ",{"score":{"name":"@s","objective":"kit"}}]
