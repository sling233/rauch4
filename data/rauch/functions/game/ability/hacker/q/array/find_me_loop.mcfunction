...
execute store result score Global comparer run data get storage game_data hacker_q_tag[0].id

# if its not @s's data, append to end of array and delete front
execute unless score @s pnum = Global comparer run data modify storage game_data hacker_q_tag append from storage game_data hacker_q_tag[0]
execute unless score @s pnum = Global comparer run data remove storage game_data hacker_q_tag[0]
