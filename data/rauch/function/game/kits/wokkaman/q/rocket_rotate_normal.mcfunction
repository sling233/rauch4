...
#calculate height
execute at @a[tag=temp] run summon marker ~ ~ ~ {Tags:["delta"]}
$execute as @e[type=marker,tag=delta] at @s rotated $(delta_angle) 0 rotated ~90 0 positioned ^ ^ ^$(roll_angle) run tp @s ~ ~ ~ ~ ~
execute store result score t_marker_z temp run data get entity @e[type=marker,tag=delta,limit=1] Pos[2] 1000
...
scoreboard players operation t_marker_z temp += t_old_y temp
# set new y value
execute store result entity @s Pos[1] double 0.001 run scoreboard players get t_marker_z temp

execute at @s run tp @s ^ ^ ^1.5
execute at @s rotated ~ -90 run tp @s ~ ~ ~ ~ ~
#execute if score t_is_grounded temp matches 1 at @s run particle dust{color:[0.4,0.4,0.4],scale:0.6} ^ ^ ^-0.3 0 0 0 0 1 force
#execute unless score t_is_grounded temp matches 1 at @s run particle small_flame ^ ^ ^-0.3 0 0 0 0 1 force
