...
$execute as @s[tag=!temp,tag=!temp2,tag=!temp3] run function rauch:settings/edit_map/edit_initialize {"map_id":$(map_id)}

execute as @s[tag=temp2] run tellraw @s {"text":"Only one Admin at a time can edit maps","color":"red"}
execute as @s[tag=temp3] run tellraw @s {"text":"Cannot edit maps while a game is running","color":"red"}
