data modify storage map_data maps prepend value {name:"Splat",id:1,red:[],blue:[],center:[],particle:-1,time:-1,weather:-1,has_launchpads:-1,disable_block_interaction:-1,is_dark:-1,version:-1,spawn_location:[]}
# remember to change the these values in update_macro as well
data modify storage map_data maps[0].version set value 6
...
data modify storage map_data maps[0].is_dark set value 0
...
execute at @s align xyz run summon marker ~70.5 ~29 ~11.5 {Tags:["mapsetup","blu"]}
