execute at @e[type=armor_stand,tag=main] run summon armor_stand ~ ~ ~ {CustomName:'{"text":"Mork"}',NoGravity:1b,Invulnerable:1b,Invisible:1b,Marker:1b,Tags:["root","setup","map"],ArmorItems:[{},{},{},{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:0,Cords:[[na],[na],[na]]}}]}
...
scoreboard players set @e[type=armor_stand,tag=map,tag=setup] mapParticle 11
