...
# when player was in a game when leaving, kick if try mode is running right now
execute if score Global game_running matches 1 if score Global mode matches 6 as @s[tag=game] run function rauch:game/framework/move_player
execute if score Global game_running matches 1 if score Global mode matches 6 as @s[tag=spectator] run function rauch:game/framework/spectate_stop
