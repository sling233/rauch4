#execute at @s unless entity @s[scores={tank=1..}] as @e[type=area_effect_cloud,tag=stun] if score @s pnum = @p pnum run kill @s
#execute at @s unless entity @s[scores={tank=1..}] run summon area_effect_cloud ~ ~ ~ {Duration:300,Tags:["setup","stun"]}
#execute at @s unless entity @s[scores={tank=1..}] run scoreboard players operation @e[type=area_effect_cloud,tag=setup,tag=stun,sort=nearest,limit=1] pnum = @s pnum
#execute at @s unless entity @s[scores={tank=1..}] run tp @e[type=area_effect_cloud,tag=setup,tag=stun,sort=nearest,limit=1] @s
#execute at @s unless entity @s[scores={tank=1..}] run tag @e[type=area_effect_cloud,tag=setup,tag=stun,sort=nearest,limit=1] remove setup
