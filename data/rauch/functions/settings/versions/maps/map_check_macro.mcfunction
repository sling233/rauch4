...
$execute if score Global map_id matches 1 unless score Global map_ver matches 3 run tellraw @a [{"text":"An Update is available for map Splat. Click ","color":"yellow"},{"text":"here","clickEvent":{"action":"run_command","value":"/function rauch:settings/spawn_map/splat/update {index:$(index)}"},"italic":true},{"text":" to update."}]
execute if score Global map_id matches 1 unless score Global map_ver matches 3 run scoreboard players set #t_map_update_found global 1
