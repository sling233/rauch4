...
tellraw @p [{"text":"Added Map \"","color":"green"},{"nbt":"CustomName","entity":"@e[type=armor_stand,tag=map,tag=root,tag=adding,limit=1]","interpret":true},{"text":"\" with internal id of "},{"score":{"name":"Global","objective":"mapId"},"color":"light_purple"}]
